...
def home(session):
    ...
    gen_containers = [
        generation_preview(gen)
        for gen in gens(limit=10, where=f"session_id == '{session['session_id']}'")
    ]
    gen_list = Div(*reversed(gen_containers), id="gen-list", cls="row")

    return Title("Image Generation Demo"), Main(
        H1("Image Generation Demo - Session"),
        P("Hello", str(session)),
        add,
        gen_list,
        cls="container",
...
def post(prompt: str, session):
    ...
    os.makedirs(folder, exist_ok=True)
    g = gens.insert(
        Generation(prompt=prompt, session_id=session["session_id"], folder=folder)
    )
