## Credit https://github.com/AnswerDotAI/fasthtml-example/blob/main/image_app_session_credits/main.py
## Credit https://testdriven.io/blog/flask-stripe-tutorial/
...
tables = database("data/gens-credit.db").t
gens = tables.gens
...
def get_balance():
    return Div(
        P(f"Global balance: {global_balance} credits"),
        id="balance",
        hx_get="/balance",
        hx_trigger="every 2s",
        hx_swap="outerHTML",
...
def post(prompt: str, session):
    ...
    if global_balance < 1:
        return Div(P("Out of balance!")), clear_input
    ...
    g = gens.insert(
        Generation(prompt=prompt, folder=folder, session_id=session["session_id"])
    )
